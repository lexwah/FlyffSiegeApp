import {
  BonusPointType, Kill, Player, PointGain, Guild, Timestamp,
} from './models';
...
const parseKill = (line: string): Kill => {
  ...
  // Timestamps are the only things that can include colons, but we should make sure that it's valid anyway
  const timeStampRegex = /^(?:[01]\d|2[0-3]):(?:[0-5]\d):(?:[0-5]\d)$/;
  const parenthesesRegex = /\[(.*?)\]/;
  const contentOfFirstParentheses = split[0].substring(0, 10);
  const match = parenthesesRegex.exec(contentOfFirstParentheses);
  let timestampString = '';
  ...
  const hasTimestamp = timeStampRegex.test(timestampString);

  // If the kill line has a timestamp, then we should extract it, else assume its the killer's guild name
  const killerString = hasTimestamp ? split[0].substring(10, split[0].length) : split[0];
  const targetString = split[1];
  const killerEntity = parsePlayer(killerString);
  const targetEntity = parsePlayer(targetString);
  const timestamp: Timestamp = hasTimestamp ? {
    raw: timestampString,
    milliseconds: new Date(`01-01-1970 ${timestampString}`).getTime()
  } : undefined;
